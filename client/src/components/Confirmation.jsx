...
const Confirmation = ({ onStatusChange }) => {
  ...
  const handleSendOtp = async (targetMobile) => {
    ...
    try {
      ...
      setError('Connection error. Is server running?', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      setError('Verification failed. Try again.', err);
    } finally {
      setLoading(false);
    }
  };
        ...
        {step === 'CHOICE' && (
            <div className="animation-fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
                <div style={{ width: '100%', textAlign: 'center' }}>
                    <p style={{ fontSize: '14px', fontWeight: '600', color: '#1F2937', marginBottom: '16px' }}>Sign in to confirm your appointment</p>
